...
test.describe('Custom Lists', () => {
  test.beforeEach(async ({ page }) => {
    ...
    // Click on "Gerenciar Listas" using test ID
  ...
  test('should create a new list collection', async ({ page }) => {
    ...
    await page.route('**/rest/v1/custom_lists*', async (route) => {
      if (route.request().method() === 'POST') {
        await route.fulfill({
          status: 201,
          contentType: 'application/json',
          body: JSON.stringify({ id: 'list-123', name: 'Lista E2E Teste', user_id: 'user-123' }),
        });
      } else {
        await route.fulfill({
          status: 200,
    ...
    await aside.locator('button').first().click();
  ...
  test('should allow opening sharing modal', async ({ page }) => {
    ...
    await expect(page.locator('.glass-card h3')).toContainText('Compartilhar Lista');
